...
import ca.umontreal.IFT2015.pqueues.DefaultComparator;
...
public class SkipListMap<K,V> extends AbstractSortedMap<K,V> {
	...
	while( p.getBelow() != null && p.getNext().getKey() == this.plusInfinite ) {
	    this.start = p;
	    this.h--;
	    p = p.getBelow();
	}
	...
	this.skipInsert( key, value, p ); // otherwise add new entry
